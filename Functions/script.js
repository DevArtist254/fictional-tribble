...
1. Create a method called 'registerNewAnswer' on the 'poll' object. The method does 2 things:
  ...
  1.2. Based on the input number, update the answers array. For example, if the option is 3, increase the value AT POSITION 3 of the array by 1. Make sure to check if the input is a number and 
      if the number makes sense (e.g answer 52 wouldn't make sense, right?)
2. Call this method whenever the user clicks the "Answer poll" button.
3. Create a method 'displayResults' which displays the poll results. The method takes a string as an input (called 'type'), which can be either 'string' or 'array'. If type is 'array', simply display 
   the results array as it is, using console.log(). This should be the default option. If type is 'string', display a string like "Poll results are 13, 2, 4, 1". 
...
function answerPoll(e) {
  
  if(ans < 0 && ans > 3) console.log("Error");

  switch (ans) {
    case 0:
      this.javascript.push(1);
      this.ans[0] = sum(this.javascript);
      break;
    case 1:
      this.python.push(1);
      this.ans[1] = sum(this.python);
      break;
    case 2:
      this.rust.push(1);
      this.ans[2] = sum(this.rust);
      break;
    case 3:
      this.Cplus.push(1);
      this.ans[3] = sum(this.Cplus);
      break;
    default:
      console.log("Hello world");
      break;
